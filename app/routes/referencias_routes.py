...
def referencias():
    referencias = referencias_service.listar_referencias()
    return render_template('temp_referencias/referencias.html', referencias = referencias)
...
def f_addReferencia():
    try:
        ...
        cod_principio = request.form["cod_principio"]
        ...
        referencias_service.insert_referencia(cod_referencia, nom_referencia, cod_principio, principio_activo, nom_comercial_ref, unidad_medida, concentracion,
...
#Ruta Ventana Editar Referencia
@bp_referencias.get('/edit_referencia/<codigo>')
def edit_referencia(codigo):
    referencia = referencias_service.listar_referencia_codigo(codigo)
    formas = formas_farmihce_service.listar_formas_ihce()
    return render_template('temp_referencias/edit_referencia.html', referencia = referencia, formas = formas)

#Ruta Metodo Guardar Referencia
@bp_referencias.post('/f_updateReferencia')
def f_updateReferencia():
    try:
        nom_referencia = request.form["nom_referencia"]
        cod_principio = request.form["cod_principio"]
        principio_activo = request.form["principio_activo"]
        ...
        referencias_service.update_referencia(nom_referencia, cod_principio, principio_activo, nom_comercial_ref, unidad_medida, concentracion,
                                              forma_farm, forma_farm_ihce, laboratorio, presentacion, grupo_referencia, registro_invima, tipo_referencia,
                                              regulado, serie_referencia, expediente_sanitario, consecutivo_exp, codigo_atc, codigo_cum, codigo_rips,
